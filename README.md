## Welcome to LaserLevel!
[Compiling LaserLevel](#compile-laserlevel) - for those that want the latest unreleased features
...
[Setup LaserLevel](#setup-laserlevel)

[Compile LaserLevel](#compile-laserlevel)

[Updating Your Repository](#updating-your-repository)

#### Setup Java
The Java JDK is used to compile LaserLevel.
...
#### Setup Git
Git is used to clone LaserLevel and update your local copy.
...
2. Navigate to a place where you want to download LaserLevel's source (eg `C:\Development\Github\Minecraft\`) by executing `cd [folder location]`. This location is known as `mcdev` from now on.
3. Execute `git clone git@github.com:Lunatrius/LaserLevel.git`. This will download LaserLevel's source into `mcdev`.
4. Right now, you should have a directory that looks something like:

***
    mcdev
    \-LaserLevel
        \-LaserLevel's files (should have build.gradle)
***

#### Compile LaserLevel
1. Execute `gradlew setupDevWorkspace`. This sets up Forge and downloads the necessary libraries to build LaserLevel. This might take some time, be patient.
    * You will generally only have to do this once until the Forge version in `build.properties` changes.
2. Execute `gradlew build`. If you did everything right, `BUILD SUCCESSFUL` will be displayed after it finishes. This should be relatively quick.
    * If you see `BUILD FAILED`, check the error output (it should be right around `BUILD FAILED`), fix everything (if possible), and try again.
3. Go to `mcdev\LaserLevel\build\libs`.
    * You should see a `.jar` file named `LaserLevel-#.#.#-#.#.#.#-universal.jar`.
